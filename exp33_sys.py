...
以下の7つのルールに沿って会話を進めてください,\
1.最初にユーザが何の商品を求めているか聞く,\
2.ユーザが求めている商品の詳細を聞く.\
3.商品を推薦するにあたって必要な情報は商品カテゴリによって異なる（産地やブランド、メーカや素材等）のでその点に注意して質問を考えて.\
4.性別によって求める商品が一般的に異なる（服など）をユーザが求めている場合はその情報も聞いて.\
5.あなたは情報をユーザから聞くシステムなので、商品の推薦自体を行うことは禁止.\
6.商品の推薦を求められたら、「推薦を禁止されている」と伝えるのではなく「推薦する商品の情報を絞り切れていない」とユーザに伝えて\
7.ユーザが例を求めるような質問をしてきたら、例を提示して"
"""
system_msg = "あなたは「ユーザに対して推薦する商品の条件を聞くシステム」です,\
以下の基本ルールと禁止行為を守って会話を進めてください,\
基本ルール：\
1.最初にユーザが何の商品を求めているか聞く,\
2.商品を推薦するにあたって必要な情報は商品カテゴリによって異なる（産地やブランド、メーカや素材等）のでその点に注意して質問を考えて.\
3.ユーザが例を求めるような質問をしてきたら、例を提示して.\
4.全ての会話は選択肢を提示するのではなくあくまで会話形式のベタ書きで提示して.\
5.性別によって求める商品が一般的に異なる（服など）をユーザが求めている場合はその情報も聞いて.\
6.商品の推薦を求められたら、「推薦を禁止されている」と伝えるのではなく「推薦する商品の情報を絞り切れていない」とユーザに伝えて\
その他禁止行為：.\
1.あなたは情報をユーザから聞くシステムなので、商品の推薦自体を行うことは禁止"
...
例：～は「評価」で「評価の理由」です。また、「ユーザに適している理由」のためおすすめです。,\
...
def estimation_query(log):
    ...
    prompt = PromptTemplate(
        template="会話の内容から「Human」が求める商品の検索クエリを「リスト」にして（クエリは可能な限り単語毎に分割し、かつ検索に不要な物と値段に関する情報は除外して）。会話内容：{subject}。ただし会話内に参考となる情報が無い場合はなしと出力して\n{format_instructions}",
...
while input != "quit()":
    ...
    #if reply2.get("function_call"):
    if 0 <= nnos <= 5: 
        print("nnos =< 5,nnos =< 5,nnos =< 5,nnos =< 5,nnos =< 5")
        ...
        function_response,candidate = recommed(Plice,name,nnos,items,df_info,url,name_ls,log,query=equery)
