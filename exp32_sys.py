...
#検索の際値段に関する内容から適切なURLを抽出できるようにする
#Functioncallingのクエリ生成部分でURLを取得できるか試してみる
#システム概要
...
2.プロンプト内の「検索件数」を絞るための質問を「各商品の情報」から考えて（産地や価格やブランド等、「各商品の情報」から具体例を提示して）.\
3.ユーザが例を求めるような質問をしてきたら、例を提示して\
4.全ての会話は選択肢を提示するのではなくあくまで会話形式のベタ書きで提示して.\
5.性別や歳によって求める商品が一般的に異なる（服など）をユーザが求めている場合はその情報も聞いて.\
推薦タイミングに関するルール\
1.「各商品の情報」の中にユーザにおすすめできそうな商品があれば直ぐに推薦を行って.\
2.「検索件数」が絞りきれても、文脈上ユーザが質問をしている場合は質問に答えてから推薦を行って\
その他禁止行為：\
1.この会話で商品を紹介することは禁止."

system_msg2 = "あなたは「推薦した商品をユーザに説明するシステム」です,\
以下の５つのルールに沿って会話を進めてください,\
...
def estimation_query(log):
    ...
    prompt = PromptTemplate(
        template="会話の内容から「Human」が求める商品の検索クエリを「リスト」にして（クエリは可能な限り単語毎に分割し、かつ検索に不要な物と値段に関する情報は除外して）。会話内容：{subject}。ただし会話内に参考となる情報が無い場合はなしと出力して。会話中に別ジャンルの商品の話題になったらそれまでのクエリは考慮しなくてよい\n{format_instructions}",
...
#各ターンで検索を行い検索結果をまとめる(値段,商品情報,URL)
...
def explanation(goods,log,system_msg2,candidate,messages):
    ...
    messages.append({"role": "assistant", "content": "お待たせいたしました。貴方におすすめの商品は以下の通りです。"})
    print(candidate)
    log += candidate
...
my_function1 = [
    {   
        ...
        "name": "recommed",
        #関数の説明
        "description": "商品の推薦や商品の紹介を行う際に「必ず」実行しなければいけない関数。商品の詳細な内容を調べることが可能。プロンプト内の商品を紹介する際にも実行しなければいけない関数",
        #関数の引数の定義
        "parameters": {
            "type": "object",
            "properties": {
