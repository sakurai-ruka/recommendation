...
while input != "quit()":
    ...
    if (len(name)>0 and len(name)/2<10) or re_try == 1 or turn ==20:
...
2.会話は簡潔に,\
3.商品を説明する時はプロンプト内にある「評価」と、以下で定義する「ユーザに適している理由」を例のように箇条書きではなくべた書きで提示して,\
「ユーザに適している理由」の定義:商品がユーザにとってどのような点で適しているか（ユーザに関する情報から生成して）,\
例：～は「評価」です。「ユーザに適している理由」なので特におすすめです。,\
